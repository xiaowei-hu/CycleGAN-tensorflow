...
def generator_resnet(image, options, reuse=False, name="generator"):
    ...
    with tf.variable_scope(name):
        ...
        def residule_block(x, dim, ks=3, s=1, name='res'):
            y = batch_norm(conv2d(x, dim, ks, s, name=name+'_c1'), name+'_bn1')
            y = batch_norm(conv2d(tf.nn.relu(y), dim, ks, s, name=name+'_c2'), name+'_bn2')
            return y + x
        ...
        c1 = tf.nn.relu(batch_norm(conv2d(image, options.gf_dim, 7, 1, name='g_e1_c'), 'g_e1_bn'))
        ...
        pred = conv2d(d2, options.output_c_dim, 7, 1, name='g_pred_c')
